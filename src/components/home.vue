                ...
                <div :class="$store.state.module.flag?'menu-collapsed_none':''">
                    <aside :class="collapsed?'menu-collapsed':'menu-expanded'">
                        ...
                        <el-menu :default-active="$route.path" class="el-menu-vertical-demo" unique-opened router
                                 v-show="!collapsed" background-color="#545c64" text-color="#fff"
                        ...
                        <ul class="el-menu el-menu-vertical-demo collapsed" v-if="collapsed" ref="menuCollapsed">
                            <li v-for="(item,index) in $router.options.routes" v-if="!item.hidden"
                                ...
                                <template v-if="!item.leaf">
                                    <div class="el-submenu__title" :id="index" style="padding-left: 20px;"
                                         @mouseover="showMenu(index,true,$event)"
                                         @mouseout="showMenu(index,false,$event)">
                                        <i :class="item.iconCls"></i>
                                    </div>
                                    <ul class="el-menu submenu" :class="'submenu-hook-'+index"
                                        @mouseover="showMenu(index,true,$event)"
                                        @mouseout="showMenu(index,false,$event)"
                                        :style="{top:setTop+'px',left:40+'px'}">
    ...
    export default {
        ...
        methods: {
            ...
            Interface() {
                ...
                if (Obj) {
                    Obj = JSON.parse(Obj);
                    this.$http.get(`/api/v1/userinfo/${Obj.Unicode}`).then(this.Biography)
                }
            },
            Biography(res) {
                console.log(res)
